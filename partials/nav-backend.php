
<!-- DESKTOP--BACKEND-NAVIGATION -->
<nav class="desktop-backend-navigation">
            <a href="#"><img class="logo" src="../images/logo_jb.webp" alt="Initialen JB in rosa, leicht kursiv"></a>
            <!-- <ul>
                <li><a href="../view/index.php">Home</a></li>
                <li><a href="../view/ueber-mich.php">Über mich</a></li>
                <li><a href="../view/projekte.php">Projekte</a></li>
            </ul> -->
        </nav>
        
        <!-- MOBILE--BACKEND- NAVIGATION-HAMBURGER -->
        <nav class="mobile-backend-navigation"> 
            <a href="#"><img src="../images/logo_jb.webp" alt="Initialen JB in rosa, leicht kursiv"></a>
            <button class="hamburger-button"><i class="fa-solid fa-bars"></i></button>
            <div class="overlay-nav">
                <button class="close-button"><i class="fa-solid fa-close"></i></button>
                <!-- <ul class="nav-links">
                    <li>
                        <a href="../view/index.php">HOME</a>
                    </li>
                    <li>
                        <a href="../view/ueber-mich.php">ÜBER MICH</a>
                    </li>
                    <li>
                        <a href="../view/projekte.php">PROJEKTE</a>
                    </li>
                    <li>
                        <a href="../view/impressum.php">IMPRESSUM</a>
                    </li>
                    <li>
                        <a href="../view/datenschutz.php">DATENSCHUTZ</a>
                    </li>
                </ul> -->
            </div>
        </nav>